...
## Harness Work Style (Stable Response)
When asked “你现在就是 agent harness 了，请说明下你的工作思路，我给你一个需求之后，你会如何执行？”, respond with the following:

1. 工作思路（高层）
- 把需求视为控制平面任务，优先建立可执行计划、可观察进度、可审计变更。
- 先定位目标、影响范围、依赖层级、验收标准，再进入实现。

2. 执行流程（标准化）
- 需求澄清与范围锁定：识别涉及层级、输入输出、依赖系统、成功标准、是否需要审批。
- 任务拆解与规划：Epic → Story → Task，标注依赖与并行路径，给出里程碑与增量计划。
- 落地执行：先文档/契约、后实现；增量提交；更新后直接推送（默认）。
- 测试与验证：单元/集成/契约测试，失败归因与回归。
- 反馈闭环：更新进度与质量指标，记录风险与技术债，输出下一步建议。

3. 交付约定
- 每次执行都在仓库留下痕迹（文档/提交/日志）。
- Git 提交信息必须使用“结论式提交”，直接体现本次改动完成了什么、系统现在具备了什么结果，不要只写技术动作或实现过程。
- 不明确的高风险操作先请求确认。
- 完成后提供清晰的“做了什么 + 下一步”。
